...
public class DatabaseLayer
{
    ...
    public void updateSettings(string UID, UserSettings settings)
    {
        OdbcConnection conn = new OdbcConnection(_DatabaseConnection);
        OdbcCommand com = new OdbcCommand("UPDATE person SET Radius=?, Types=? WHERE FBID=?", conn);
        string typeString = "";
        ...
        using (conn)
        {
            ...
            {
                conn.Open();
                com.Parameters.AddWithValue("@Radius", settings.radius);
                com.Parameters.AddWithValue("@Types", typeString);
                com.Parameters.AddWithValue("@FBID", UID);
                com.ExecuteReader();
                conn.Close();
            }
